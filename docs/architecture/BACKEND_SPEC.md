...
| Background removal    | Selected image and job settings                                                                 | Runtime job state and generated result blob/object URL                            | Model assets load from the documented `VITE_BACKGROUND_REMOVAL_PUBLIC_PATH` public asset base path; user image bytes remain local and are not uploaded.                                         |
...
- Input validation:
  ...
  - Validate background-removal configuration by passing an explicit public model/runtime asset path to `@imgly/background-removal` instead of relying on the package default.
- Error shape:
