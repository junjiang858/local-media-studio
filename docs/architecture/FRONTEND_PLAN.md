...
| `MediaAssetCard`             | Compact thumbnail, type, name, file size, generated-result tag, and error badge                            | `apps/web/src/components/media-library`               | Media library             | Stateless display plus selected/disabled props                                 |
...
| `ImageCropOverlay`           | Konva-backed crop rectangle and custom ratio interaction                                                   | `apps/web/src/components/preview`                     | Image workflow            | Local crop drag state; committed crop rectangle lives in image edit state      |
...
| `WatermarkControls`          | Text/image watermark placement, opacity, and layer controls                                                | `apps/web/src/components/editor`                      | Image workflow            | Draft form state local; committed watermark state in store/core                |
...
- State pattern:
  ...
  - Project-owned image edit history in media-core for undo/redo/reset; zundo remains an approved fallback if broader temporal store behavior is needed later.
...
- Interaction:
  ...
  - Media library cards intentionally show compact session metadata only: thumbnail, file name, media type, file size, generated-result tag when relevant, and error state. Full format, MIME type, dimensions, and duration live in the selected preview metadata popover to preserve library scan density. Do not render static labels such as "privacy status" when they do not represent a changing state or actionable control.
  ...
  - Watermarks are layers. Text and image watermarks can be moved, transformed, hidden, deleted, adjusted for opacity, and exported through the same layer renderer as annotations.
  ...
  - Video processing surfaces show a non-blocking large-media warning before preview/export when the source is at least 80 MB or at least 3 minutes long. The warning explains that browser-local ffmpeg can be slow and may fail depending on memory and codec support.
  - MOV, MKV, and AVI outputs are allowed as downloadable local exports, but the UI must warn that generated results in those containers may not play in the browser preview. MP4 and WebM remain the recommended preview-safe formats.
...
5. Image interactive editing: Konva crop rectangle, annotation/watermark layers, layer selection, drag, transform, hide, delete, opacity controls, and export parity.
